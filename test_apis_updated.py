"""
Updated test script to verify API keys using tweepy and PRAW
"""
...
def test_twitter_api():
    """Test Twitter/X API connection using tweepy"""
    try:
        import tweepy
...
def test_reddit_api():
    ...
    try:
        ...
        logger.info(f"Testing Reddit API with client ID: {client_id[:10]}...")
...
if __name__ == "__main__":
    ...
    print(f"FRED API: {'✅ Working' if fred_result else '❌ Failed'}")
    print(f"Twitter API: {'✅ Working' if twitter_result else '❌ Failed'}")
    print(f"Reddit API: {'✅ Working' if reddit_result else '❌ Failed'}")
