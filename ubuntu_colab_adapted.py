#!/usr/bin/env python3
"""
Ubuntu PySpark Stock Prediction System
Adapted from Google Colab version for Ubuntu with Hadoop/Spark

Usage: spark-submit ubuntu_colab_adapted.py [ticker] [days_to_predict]
"""
...
import numpy as np
import argparse
random = np.random  # Use numpy's random for consistency
...
try:
    ...
    print("TextBlob not available, sentiment analysis will be limited")
...
class UbuntuPySparkStockPredictor:
    ...
    def __init__(self):
        ...
        self.setup_ubuntu_environment()
    
    def setup_ubuntu_environment(self):
        """Setup Ubuntu environment for PySpark"""
        print("🔧 Setting up Ubuntu environment...")
        
        try:
            # Create Spark session for Ubuntu
            self.spark = SparkSession.builder \
                .appName("UbuntuStockPredictor") \
    ...
    def predict_future_prices(self, model, feature_df: DataFrame, days: int = 5):
        ...
        def clean_column_name(col_name):
            ...
            if isinstance(col_name, str):
                ...
                cleaned = re.sub(r'^\d+(\. |\`\. )', '', str(col_name))
    ...
    def run_prediction_pipeline(self, ticker: str, prediction_days: int = 5):
        ...
        try:
            ...
            # For predictions, we use the feature_df directly since it already has all features
            # Make predictions
            predictions = self.predict_future_prices(best_model, feature_df, prediction_days)
...
# Main execution function for Ubuntu
def run_ubuntu_prediction(ticker="TSLA", days=5):
    """
    Main function to run on Ubuntu
    
    Usage:
    run_ubuntu_prediction("TSLA", 5)  # Predict TSLA for next 5 days
    """
    predictor = None
    try:
        print("🌟 Ubuntu PySpark Stock Prediction System")
        print("=" * 50)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Ubuntu PySpark Stock Prediction System")
    parser.add_argument("--ticker", "-t", default="TSLA", help="Stock ticker symbol (default: TSLA)")
    parser.add_argument("--days", "-d", type=int, default=5, help="Number of days to predict (default: 5)")
    
    args = parser.parse_args()
    
    # Run prediction
    results = run_ubuntu_prediction(args.ticker, args.days)
    print(f"\n📋 Final Results: {results['ticker']} - Model: {results['model_used']} - R²: {results['model_performance']['r2']:.4f}")
    print(f"🔮 Predictions: {len(results['predictions'])} days completed")